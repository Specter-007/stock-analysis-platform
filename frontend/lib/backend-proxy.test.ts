...
describe("proxyToBackend", () => {
  ...
  it("does not leak any temporary debug headers into the response", async () => {
    // A prior debugging pass added x-debug-* diagnostic headers
    // (safe - names/counts/booleans only, never used to leak actual
    // token values) to pinpoint a production failure. Once the real root
    // cause was confirmed and fixed, those headers were removed - this
    // guards against them silently coming back.
    const mockResponse = new Response("{}", {
      status: 200,
      headers: [["set-cookie", "session_token=abc123; HttpOnly; Path=/; SameSite=lax"]],
    });
    ...
    for (const key of res.headers.keys()) {
      expect(key.toLowerCase().startsWith("x-debug-")).toBe(false);
    }
  });
