...
def test_stop_loss_triggers_automatic_exit(fixed_price):
    paper_trading_service.execute_trade("sl1", "AAPL", "BUY", 10, stop_loss_percent=5.0)
    fixed_price["AAPL"] = 94.0  # more than 5% below entry of 100
    view = paper_trading_service.get_portfolio("sl1")
    assert view.positions == []
    exit_trade = next(t for t in view.trades if t.action == "SELL")
    assert exit_trade.exit_reason == "STOP_LOSS"


def test_stop_loss_does_not_trigger_above_threshold(fixed_price):
    paper_trading_service.execute_trade("sl2", "AAPL", "BUY", 10, stop_loss_percent=5.0)
    fixed_price["AAPL"] = 96.0  # only 4% below entry
    view = paper_trading_service.get_portfolio("sl2")
    assert len(view.positions) == 1


def test_take_profit_triggers_automatic_exit(fixed_price):
    paper_trading_service.execute_trade("tp1", "AAPL", "BUY", 10, take_profit_percent=10.0)
    fixed_price["AAPL"] = 112.0  # more than 10% above entry
    view = paper_trading_service.get_portfolio("tp1")
    assert view.positions == []
    exit_trade = next(t for t in view.trades if t.action == "SELL")
    assert exit_trade.exit_reason == "TAKE_PROFIT"


def test_trailing_stop_triggers_after_price_pulls_back_from_peak(fixed_price):
    paper_trading_service.execute_trade("ts1", "AAPL", "BUY", 10, trailing_stop_percent=5.0)
    fixed_price["AAPL"] = 130.0  # price rallies - raises the trailing peak
    paper_trading_service.get_portfolio("ts1")  # triggers peak update
    fixed_price["AAPL"] = 122.0  # pulls back >5% from the 130 peak (not from entry)
    view = paper_trading_service.get_portfolio("ts1")
    assert view.positions == []
    exit_trade = next(t for t in view.trades if t.action == "SELL")
    assert exit_trade.exit_reason == "TRAILING_STOP"


def test_trailing_stop_does_not_trigger_on_pullback_from_entry_if_still_above_peak_threshold(fixed_price):
    paper_trading_service.execute_trade("ts2", "AAPL", "BUY", 10, trailing_stop_percent=20.0)
    fixed_price["AAPL"] = 130.0
    paper_trading_service.get_portfolio("ts2")
    fixed_price["AAPL"] = 110.0  # down from peak of 130 by ~15% - inside the 20% trailing band
    view = paper_trading_service.get_portfolio("ts2")
    assert len(view.positions) == 1


def test_no_exit_controls_means_no_auto_exit_regardless_of_price_move(fixed_price):
    paper_trading_service.execute_trade("ne1", "AAPL", "BUY", 10)
    fixed_price["AAPL"] = 1.0  # huge drop, but no stop-loss was set
    view = paper_trading_service.get_portfolio("ne1")
    assert len(view.positions) == 1


def test_manual_sell_reason_is_manual_paper_exit(fixed_price):
    paper_trading_service.execute_trade("m1", "AAPL", "BUY", 10)
    view = paper_trading_service.execute_trade("m1", "AAPL", "SELL", 10)
    assert view.trades[-1].exit_reason == "MANUAL_PAPER_EXIT"


def test_close_all_positions_uses_end_of_test_reason(fixed_price):
    paper_trading_service.execute_trade("ca1", "AAPL", "BUY", 5)
    paper_trading_service.execute_trade("ca1", "MSFT", "BUY", 2)
    view = paper_trading_service.close_all_positions("ca1")
    assert view.positions == []
...
def test_trade_journal_reports_gross_fees_slippage_net_separately(fixed_price):
    paper_trading_service.execute_trade("cost1", "AAPL", "BUY", 10)
    fixed_price["AAPL"] = 110.0
    view = paper_trading_service.execute_trade("cost1", "AAPL", "SELL", 10)
...
def test_fixed_capital_percent_sizing_via_execute_trade(fixed_price):
    view = paper_trading_service.execute_trade(
        "size1", "AAPL", "BUY", shares=None, sizing_mode="FIXED_CAPITAL_PERCENT", capital_percent=10.0
    )
...
def test_risk_percent_sizing_via_execute_trade(fixed_price):
    view = paper_trading_service.execute_trade(
        "size2", "AAPL", "BUY", shares=None, sizing_mode="RISK_PERCENT",
...
def test_sizing_without_shares_or_mode_defaults_to_fixed_shares_and_fails_without_shares(fixed_price):
    with pytest.raises(paper_trading_service.PaperTradingError):
        paper_trading_service.execute_trade("size3", "AAPL", "BUY", shares=None)


# ------------------------------------------------------------ Risk dashboard

def test_risk_dashboard_flags_high_concentration(fixed_price, mocked_sector):
    paper_trading_service.execute_trade("risk1", "AAPL", "BUY", 50)  # ~$5,005 of $10,000 portfolio
    view = paper_trading_service.get_portfolio("risk1")
    result = paper_risk.assess_portfolio_risk(view)
    assert any("HIGH_CONCENTRATION" in w for w in result.warnings)


def test_risk_dashboard_no_warnings_for_diversified_small_positions(fixed_price, mocked_sector):
    ...
    paper_trading_service.execute_trade("risk2", "AAPL", "BUY", 3)  # Technology, ~$300
    paper_trading_service.execute_trade("risk2", "TSLA", "BUY", 6)  # Consumer Cyclical, ~$300
    paper_trading_service.execute_trade("risk2", "JNJ", "BUY", 2)  # Healthcare, ~$300
    view = paper_trading_service.get_portfolio("risk2")
    result = paper_risk.assess_portfolio_risk(view)
    assert not any("HIGH_CONCENTRATION" in w for w in result.warnings)


def test_risk_dashboard_sector_concentration_sums_to_100(fixed_price, mocked_sector):
    paper_trading_service.execute_trade("risk3", "AAPL", "BUY", 5)
    paper_trading_service.execute_trade("risk3", "TSLA", "BUY", 5)
    view = paper_trading_service.get_portfolio("risk3")
    result = paper_risk.assess_portfolio_risk(view)
    assert sum(result.sector_concentration.values()) == pytest.approx(100.0, abs=0.5)


def test_risk_dashboard_low_cash_warning(fixed_price, mocked_sector):
    ...
    paper_trading_service.execute_trade(
        "risk4", "AAPL", "BUY", shares=None, sizing_mode="FIXED_CAPITAL_PERCENT",
        capital_percent=98.0, max_position_percent=100.0,
    )
    view = paper_trading_service.get_portfolio("risk4")
    result = paper_risk.assess_portfolio_risk(view)
    assert any("LOW_CASH" in w for w in result.warnings)


def test_risk_dashboard_no_warnings_for_all_cash_portfolio(fixed_price, mocked_sector):
    view = paper_trading_service.get_portfolio("risk5")
    result = paper_risk.assess_portfolio_risk(view)
