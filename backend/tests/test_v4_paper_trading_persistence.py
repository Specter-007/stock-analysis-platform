...
def test_no_snapshot_recorded_when_market_data_unavailable(fixed_price, monkeypatch):
    monkeypatch.setattr(paper_trading_service, "_latest_real_trading_day", lambda: (None, None))
    paper_trading_service.execute_trade("p1", "AAPL", "BUY", 10)
    history = paper_trading_service.get_equity_history("p1")
    assert history.snapshots == []


def test_first_snapshot_recorded_on_query(fixed_price, fake_trading_day):
    paper_trading_service.execute_trade("p2", "AAPL", "BUY", 10)
    history = paper_trading_service.get_equity_history("p2")
...
def test_repeated_query_same_trading_day_does_not_duplicate(fixed_price, fake_trading_day):
    paper_trading_service.execute_trade("p3", "AAPL", "BUY", 10)
    paper_trading_service.get_portfolio("p3")
    paper_trading_service.get_portfolio("p3")
    paper_trading_service.get_portfolio("p3")
    history = paper_trading_service.get_equity_history("p3")
    assert len(history.snapshots) == 1


def test_new_trading_day_appends_not_replaces(fixed_price, fake_trading_day):
    paper_trading_service.execute_trade("p4", "AAPL", "BUY", 10)
    paper_trading_service.get_portfolio("p4")  # day 1 snapshot
    ...
    paper_trading_service.get_portfolio("p4")  # day 2 snapshot

    history = paper_trading_service.get_equity_history("p4")
    assert [s.date for s in history.snapshots] == ["2026-01-05", "2026-01-06"]


def test_daily_pnl_is_change_since_previous_recorded_snapshot(fixed_price, fake_trading_day):
    paper_trading_service.execute_trade("p5", "AAPL", "BUY", 10)
    day1 = paper_trading_service.get_equity_history("p5").snapshots[0]

    fake_trading_day["day"] = dt.date(2026, 1, 6)
    fixed_price["AAPL"] = 120.0
    paper_trading_service.get_portfolio("p5")
    day2 = paper_trading_service.get_equity_history("p5").snapshots[1]

    assert day2.previous_snapshot_date == "2026-01-05"
    assert day2.daily_pnl == pytest.approx(day2.equity - day1.equity)


def test_benchmark_value_tracks_spy_proportionally_from_basis_day(fixed_price, fake_trading_day):
    paper_trading_service.execute_trade("p6", "AAPL", "BUY", 10)
    paper_trading_service.get_portfolio("p6")  # basis: SPY=500 -> benchmark_value=10000

    fake_trading_day["day"] = dt.date(2026, 1, 6)
    fake_trading_day["spy_close"] = 550.0  # SPY up 10% from basis
    paper_trading_service.get_portfolio("p6")

    history = paper_trading_service.get_equity_history("p6")
    assert history.snapshots[1].benchmark_value == pytest.approx(11_000.0)  # 10% up from 10000 basis


def test_cumulative_return_percent_matches_equity_vs_starting_capital(fixed_price, fake_trading_day):
    paper_trading_service.execute_trade("p7", "AAPL", "BUY", 10)
    view = paper_trading_service.get_portfolio("p7")
    history = paper_trading_service.get_equity_history("p7")
    snap = history.snapshots[0]
    expected = (view.current_value / 10_000.0 - 1.0) * 100.0
    assert snap.cumulative_return_percent == pytest.approx(round(expected, 2))


def test_reset_clears_equity_history_and_benchmark_basis(fixed_price, fake_trading_day):
    paper_trading_service.execute_trade("p8", "AAPL", "BUY", 10)
    paper_trading_service.get_portfolio("p8")
    assert len(paper_trading_service.get_equity_history("p8").snapshots) == 1

    paper_trading_service.reset("p8")
    history = paper_trading_service.get_equity_history("p8")
...
def test_snapshot_history_never_shrinks_across_many_queries(fixed_price, fake_trading_day):
    paper_trading_service.execute_trade("p9", "AAPL", "BUY", 10)
    days = [dt.date(2026, 1, d) for d in range(5, 11)]
    counts = []
    for d in days:
        fake_trading_day["day"] = d
        paper_trading_service.get_portfolio("p9")
        counts.append(len(paper_trading_service.get_equity_history("p9").snapshots))
    assert counts == sorted(counts)
    assert counts[-1] == len(days)


def test_invested_value_equals_equity_minus_cash(fixed_price, fake_trading_day):
    paper_trading_service.execute_trade("p10", "AAPL", "BUY", 10)
    view = paper_trading_service.get_portfolio("p10")
    snap = paper_trading_service.get_equity_history("p10").snapshots[0]
