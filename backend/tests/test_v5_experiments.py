...
def isolated_store(tmp_path, monkeypatch):
    monkeypatch.setattr(store, "_DATA_DIR", tmp_path / "experiments")
...
def test_create_experiment_starts_in_draft_with_fingerprint(ohlcv_long):
    config = _config()
    exp = service.create_experiment(config, name="Test Experiment")
    assert exp.status == STATUS_DRAFT
    assert exp.fingerprint
    assert exp.id.startswith("exp_")


def test_created_experiment_is_persisted_and_reloadable(ohlcv_long):
    exp = service.create_experiment(_config(), name="Persisted")
    reloaded = service.get_experiment(exp.id)
    assert reloaded is not None
    assert reloaded.name == "Persisted"
    assert reloaded.config.tickers == ["AAA"]


def test_get_nonexistent_experiment_returns_none():
    assert service.get_experiment("exp_doesnotexist") is None


# ------------------------------------------------------------ Fingerprint

def test_identical_configs_produce_identical_fingerprints():
    a = compute_fingerprint(_config())
    b = compute_fingerprint(_config())
    assert a == b


def test_different_capital_produces_different_fingerprint():
    a = compute_fingerprint(_config())
    b = compute_fingerprint(_config(initial_capital=20_000.0))
    assert a != b


def test_different_model_version_produces_different_fingerprint():
    a = compute_fingerprint(_config(model_version="1.0"))
    b = compute_fingerprint(_config(model_version="1.1"))
    assert a != b


def test_ticker_order_does_not_affect_fingerprint():
    a = compute_fingerprint(_config(tickers=["AAA", "BBB"]))
    b = compute_fingerprint(_config(tickers=["BBB", "AAA"]))
    assert a == b


def test_notes_and_tags_do_not_affect_fingerprint():
    exp_a = service.create_experiment(_config(), name="A", notes="hypothesis one")
    exp_b = service.create_experiment(_config(), name="B", notes="a completely different note", tags=["baseline"])
    assert exp_a.fingerprint == exp_b.fingerprint


def test_fingerprint_format_is_dash_grouped_hex():
...
def test_running_experiment_does_not_mutate_global_config_defaults(ohlcv_long):
    from app.config import MODEL_VERSION_CURRENT

    exp = service.create_experiment(_config(model_version="1.0"), name="Immutable test")
...
def test_experiment_config_unchanged_after_running(ohlcv_long):
    exp = service.create_experiment(_config(), name="Config check")
...
def test_reopening_experiment_shows_unchanged_configuration(ohlcv_long):
    exp = service.create_experiment(_config(initial_capital=15_000.0), name="Reopen test")
    service.run_experiment(exp.id, price_data=_price_data(ohlcv_long), benchmark_df=ohlcv_long, tickers_unavailable={})
    reloaded = service.get_experiment(exp.id)
...
def test_run_experiment_backtest_only_reaches_validated(ohlcv_long):
    start, end = _date_range(ohlcv_long)
    exp = service.create_experiment(_config(start_date=start, end_date=end), name="Backtest only")
    ran = service.run_experiment(exp.id, price_data=_price_data(ohlcv_long), benchmark_df=ohlcv_long, tickers_unavailable={})
...
def test_run_experiment_backtest_failure_sets_failed_status(ohlcv_long):
    ...
    exp = service.create_experiment(
        _config(start_date=ohlcv_long.index[-5].date().isoformat(), end_date=ohlcv_long.index[-1].date().isoformat()),
        name="Too short",
    )
    ran = service.run_experiment(exp.id, price_data=_price_data(ohlcv_long), benchmark_df=ohlcv_long, tickers_unavailable={})
...
def test_run_experiment_walk_forward_insufficient_history_stays_completed_not_validated(ohlcv_long):
    start, end = _date_range(ohlcv_long)
    exp = service.create_experiment(
        _config(start_date=start, end_date=end, run_walk_forward=True),
...
def test_data_provenance_recorded(ohlcv_long):
    start, end = _date_range(ohlcv_long)
    exp = service.create_experiment(_config(start_date=start, end_date=end), name="Provenance")
    ran = service.run_experiment(
        exp.id, price_data=_price_data(ohlcv_long), benchmark_df=ohlcv_long,
...
def test_running_experiment_persists_running_status_before_completion(monkeypatch, ohlcv_long):
    ...
    """
    start, end = _date_range(ohlcv_long)
    exp = service.create_experiment(_config(start_date=start, end_date=end), name="Crash test")

    captured_statuses = []
    original_save = store.save_experiment

    def spy_save(experiment):
        captured_statuses.append(experiment.status)
        original_save(experiment)
...
def test_portfolio_experiment_runs_backtest(ohlcv_long, ohlcv_uptrend, monkeypatch):
    ...
    exp = service.create_experiment(config, name="Portfolio test")
...
def test_portfolio_experiment_flags_unsupported_validations(ohlcv_long, ohlcv_uptrend, monkeypatch):
    from app.backtesting import portfolio as portfolio_module
    monkeypatch.setattr(portfolio_module, "_sector_for", lambda t: "Technology")
    ...
    exp = service.create_experiment(config, name="Portfolio unsupported validations")
...
def test_duplicate_experiment_has_same_fingerprint_new_id(ohlcv_long):
    original = service.create_experiment(_config(), name="Original")
    duplicate = service.duplicate_experiment(original.id)
...
def test_archive_experiment_hides_from_default_listing(ohlcv_long):
    exp = service.create_experiment(_config(), name="To Archive")
    service.archive_experiment(exp.id, archived=True)
    active = service.list_experiments(include_archived=False)
    assert exp.id not in [row["experiment"].id for row in active]
    all_rows = service.list_experiments(include_archived=True)
    assert exp.id in [row["experiment"].id for row in all_rows]


# ------------------------------------------------------ Data-mining warning

def test_data_mining_warning_flagged_past_threshold(ohlcv_long):
    from app.config import EXPERIMENT_DATA_MINING_WARNING_THRESHOLD

    config = _config()
    for i in range(EXPERIMENT_DATA_MINING_WARNING_THRESHOLD):
        service.create_experiment(config, name=f"Repeat {i}")
...
def test_no_data_mining_warning_below_threshold(ohlcv_long):
    service.create_experiment(_config(), name="Solo")
    rows = service.list_experiments()
...
def test_compare_warns_on_mismatched_periods(ohlcv_long):
    a = service.create_experiment(_config(start_date="2018-01-01", end_date="2024-01-01"), name="Long period")
    b = service.create_experiment(_config(start_date="2023-01-01", end_date="2024-01-01"), name="Short period")
    result = service.compare_experiments([a.id, b.id])
    assert any("different length" in w for w in result["warnings"])


def test_compare_warns_on_mismatched_model_versions(ohlcv_long):
    a = service.create_experiment(_config(model_version="1.0"), name="v1.0")
    b = service.create_experiment(_config(model_version="1.1"), name="v1.1")
    result = service.compare_experiments([a.id, b.id])
    assert any("model versions" in w for w in result["warnings"])


def test_compare_no_warnings_for_matching_configs(ohlcv_long):
    a = service.create_experiment(_config(), name="A")
    b = service.create_experiment(_config(), name="B")
    result = service.compare_experiments([a.id, b.id])
    assert result["warnings"] == []


def test_compare_raises_for_missing_experiment(ohlcv_long):
    a = service.create_experiment(_config(), name="A")
    with pytest.raises(ValueError):
        service.compare_experiments([a.id, "exp_missing"])


# ----------------------------------------------------- Forward simulation

def test_start_forward_simulation_single_ticker(ohlcv_long, monkeypatch):
    ...
    exp = service.create_experiment(_config(initial_capital=25_000.0), name="Forward test")
    ran = service.start_forward_simulation(exp.id)
    assert ran.status == STATUS_PAPER_FORWARD_TEST
    assert ran.forward_portfolio_id == exp.id

    portfolio = paper_trading_service.get_portfolio(ran.forward_portfolio_id)
    assert portfolio.starting_capital == 25_000.0


def test_start_forward_simulation_rejects_portfolio_experiments(ohlcv_long):
    exp = service.create_experiment(_config(tickers=["AAA", "BBB"]), name="Portfolio forward")
...
def test_corrupted_experiment_file_returns_none_not_crash(tmp_path, monkeypatch):
    monkeypatch.setattr(store, "_DATA_DIR", tmp_path / "experiments")
    exp = service.create_experiment(_config(), name="Corruptible")
    path = (tmp_path / "experiments" / f"{exp.id}.json")
    path.write_text("{not valid json", encoding="utf-8")
    assert store.load_experiment(exp.id) is None


def test_save_is_atomic_no_leftover_tmp_file(tmp_path, monkeypatch):
    monkeypatch.setattr(store, "_DATA_DIR", tmp_path / "experiments")
    exp = service.create_experiment(_config(), name="Atomic")
    tmp_files = list((tmp_path / "experiments").glob("*.tmp"))
    assert tmp_files == []


def test_full_round_trip_preserves_every_experiment_field(tmp_path, monkeypatch, ohlcv_long):
    ...
    start, end = _date_range(ohlcv_long)
    exp = service.create_experiment(
        _config(start_date=start, end_date=end, run_out_of_sample=True), name="Round trip", notes="note", tags=["baseline"],
    )
    ran = service.run_experiment(exp.id, price_data=_price_data(ohlcv_long), benchmark_df=ohlcv_long, tickers_unavailable={})
    service.archive_experiment(ran.id, archived=True)

    reloaded = store.load_experiment(ran.id)
    assert vars(reloaded) == vars(store.load_experiment(ran.id))  # stable across repeated loads
    for field_name in ("id", "name", "created_at", "status", "fingerprint", "notes", "tags", "archived", "reproduced_from"):
        assert getattr(reloaded, field_name) == getattr(store.load_experiment(ran.id), field_name)
